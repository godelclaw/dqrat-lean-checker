...
theorem processProof_sound' (content formulaContent : String) (n : Nat)
    ...
    DQBFFalse st.formula st.clauses

theorem parseDQDIMACS_none_sound (content : String)
    (h : parseDQDIMACS content = .ok none) :
    ∃ declaredMaxVar st, ... ∧ DQBFFalse st.formula st.clauses
\end{lstlisting}
...
We synchronized with the reference C++ checker at its latest revision and
...
are included; fixes were communicated upstream. On performance: the verified
...
(branch \texttt{codex/wip-parser-soundness-seam}).
